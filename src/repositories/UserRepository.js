...
class UsersRepository extends BaseRepository {
  ...
  async getUserCardDetails(userId) {
    const lookups = [
      ...
      {
        $lookup: {
          from: 'anks', // Collection for Ranks
          localField: '_id',
          foreignField: 'userId',
          as: 'rankData'
        }
    ...
    const projection = {
      _id: 0,
      xHandle: 1,
      'gemaScoreData.gemaScore': { $arrayElemAt: ['$gemaScoreData.gemaScore', 0] },
      'walletData.walletAddress': { $arrayElemAt: ['$walletData.walletAddress', 0] },
      'walletData.walletUserName': { $arrayElemAt: ['$walletData.walletUserName', 0] },
      'rankData.rank': { $arrayElemAt: ['$rankData.rank', 0] },
      'rankData.refillSpeed': { $arrayElemAt: ['$rankData.refillSpeed', 0] }
    };

    return await super.aggregationQueryByUserId('userId', userId, lookups, projection);
  }
}
